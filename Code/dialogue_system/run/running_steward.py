...
rs_success_count = {}
rs_absolute_success_count = {}
rs_total_reward = {}
rs_num_of_true_slots = {}
rs_num_of_implicit_slots = {}
rs_real_implicit_slots = {}
rs_total_turns = {}
rs_evaluate_session_number = {}
...
class RunningSteward(object):
    ...
    def evaluate_model(self, dataset, index):
        ...
        global rs_absolute_success_count
        global rs_total_reward
        global rs_num_of_true_slots
        global rs_num_of_implicit_slots
        global rs_real_implicit_slots
        global rs_total_turns
        global rs_evaluate_session_number

        for i in range(1, 10):
            if(i not in rs_success_count):
                 rs_success_count[i] = 0
            if(i not in rs_absolute_success_count):
                 rs_absolute_success_count[i] = 0
            if(i not in rs_total_reward):
                 rs_total_reward[i] = 0
            if(i not in rs_num_of_true_slots):
                 rs_num_of_true_slots[i] = 0
            if(i not in rs_num_of_implicit_slots):
                 rs_num_of_implicit_slots[i] = 0
            if(i not in rs_real_implicit_slots):
                 rs_real_implicit_slots[i] = 0
            if(i not in rs_total_turns):
                 rs_total_turns[i] = 0
            if(i not in rs_evaluate_session_number):
                 rs_evaluate_session_number[i] = 0

        for goal_index in evaluate_session_index:
            group_id = self.dialogue_manager.initialize(dataset=dataset, goal_index=goal_index)
            episode_over = False
            cur_reward = 0
            while episode_over == False:
                reward, episode_over, dialogue_status, slots_proportion_list, topk = self.dialogue_manager.next(
                    save_record=False,greedy_strategy=False, index=index)
                total_reward += reward
                cur_reward += reward
            assert len(slots_proportion_list)>0

            rs_total_reward[group_id] = rs_total_reward[group_id] + cur_reward
            rs_num_of_true_slots[group_id] = rs_num_of_true_slots[group_id] + slots_proportion_list[0]
            rs_num_of_implicit_slots[group_id] = rs_num_of_implicit_slots[group_id] + slots_proportion_list[1]
            rs_real_implicit_slots[group_id] = rs_real_implicit_slots[group_id] + slots_proportion_list[2]
            rs_total_turns[group_id] = rs_total_turns[group_id] + self.dialogue_manager.state_tracker.turn
            rs_evaluate_session_number[group_id] = rs_evaluate_session_number[group_id] + 1
            ...
            if dialogue_status == dialogue_configuration.DIALOGUE_STATUS_SUCCESS:
                rs_success_count[group_id] = rs_success_count[group_id] + 1
                success_count += 1
                if self.dialogue_manager.inform_wrong_disease_count == 0:
                    rs_absolute_success_count[group_id] = rs_absolute_success_count[group_id] + 1
                    absolute_success_count += 1
        ...
        ares_success_rate = 0
        ares_absolute_success_rate = 0
        ares_average_reward = 0
        ares_average_turn = 0
        ares_match_rate = 0
        ares_match_rate2 = 0

        for i in range(1, 10):
            if(rs_evaluate_session_number[i] == 0):
                res_success_rate[i] = math.inf
                res_absolute_success_rate[i] = math.inf
                res_average_reward[i] = math.inf
                res_average_turn[i] = math.inf
            else:
                res_success_rate[i] = float("%.3f" % (float(rs_success_count[i]) / rs_evaluate_session_number[i]))
                res_absolute_success_rate[i] = float("%.3f" % (float(rs_absolute_success_count[i]) / rs_evaluate_session_number[i]))
                res_average_reward[i] = float("%.3f" % (float(rs_total_reward[i]) / rs_evaluate_session_number[i]))
                res_average_turn[i] = float("%.3f" % (float(rs_total_turns[i]) / rs_evaluate_session_number[i]))
                ares_success_rate += res_success_rate[i]
                ares_absolute_success_rate += res_absolute_success_rate[i]
                ares_average_reward += res_average_reward[i]
                ares_average_turn += res_average_turn[i]

            if(rs_num_of_implicit_slots[i] > 0):
                res_match_rate[i] = float("%.3f" % (float(rs_num_of_true_slots[i]) / rs_num_of_implicit_slots[i]))
                ares_match_rate += res_match_rate[i]
            else:
                res_match_rate[i] = 0

            if(rs_real_implicit_slots[i] == 0):
                res_match_rate2[i] = math.inf
            else:
                res_match_rate2[i] = float("%.3f" % (float(rs_num_of_true_slots[i]) / rs_real_implicit_slots[i]))
                ares_match_rate2 += res_match_rate2[i]

        print("RS Success: ", res_success_rate)
        print(ares_success_rate/9)
        print("RS Abs Success: ", res_absolute_success_rate)
        print(ares_absolute_success_rate/9)
        print("RS Reward: ", res_average_reward)
        print(ares_average_reward/9)
        print("RS Turn: ", res_average_turn)
        print(ares_average_turn/9)
        print("RS Match Rate: ", res_match_rate)
        print(ares_match_rate/9)
        print("RS Match Rate2: ", res_match_rate2)
        print(ares_match_rate2/9)
        print("RS Session Number: ", rs_evaluate_session_number)
        ...
        if self.parameter.get("classifier_type")=="deep_learning" and self.parameter.get("disease_as_action") == False:
            res2 = self.dialogue_manager.train_deep_learning_classifier(epochs=20)
        res["loss"] = res2["1"]
        res["acc"] = res2["2"]
