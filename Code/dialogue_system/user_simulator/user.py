...
class User(object):
    ...
    def initialize(self, dataset, goal_index=None):
        ...
        #id = self.group_id[self.goal["disease_tag"]]
        id = 1
    ...
    def _init(self,dataset, goal_index=None):
        ...
        #self.group_id = pickle.load(open("D/group_id.p", 'rb'))
        #fslot_set = pickle.load(open(self.parameter.get("fslot_set"),'rb'))
        slot_set = pickle.load(open(self.parameter.get("slot_set"),'rb'))
        slot_set.pop('disease')

        #Non_RS = set(slot_set)-set(fslot_set)


        #now = Non_RS.intersection(set(self.goal['goal']['implicit_inform_slots'].keys()))
        #print('Amg Symm:',now)
    ...
    def _response_inform(self, agent_action):
        ...
        #         res[i] = math.inf
        #     else:
        #         res[i] = float("%.3f" % (float(succ[i]) / tot[i]))
        #         ares += res[i]
        # print("User Success: ",res)
        # print(ares/9)
        # print("User Number of Success: ",succ)
        # print("User Total: ",tot)
