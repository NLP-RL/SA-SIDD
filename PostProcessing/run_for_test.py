...
parser.add_argument("--save_performance_interval",dest="save_performance_interval", type=int, default = 1, help="performance interval (int)")
...
parser.add_argument("--saved_model", dest="saved_model", type=str, default="./../../model/DQN/checkpoint/0606035459_agenthrljoint2_T22_ss100_lr0.0005_RFS66_RFF0_RFNCY0.0_RFIRS0_RFRA-44_RFRMT-66_mls0_gamma0.95_gammaW0.9_epsilon0.1_awd0_crs0_hwg0_wc0_var0_sdai0_wfrs44.0_dtft0_ird0_ubc0.985_lbc1e-10_dataLabel_All_RID0/")
...
parser.add_argument("--fslot_set", dest="fslot_set", type=str, default=file0+'/fslot_set.p',help='path and filename of the slots set')
...
def run(parameter):
    ...
    if train_mode is True: # Train
        ...
        pickle.dump(obj = topk, file = open('topk.p', 'wb'), protocol = 2)
        s_r /= 5
        m_r /= 5
        r_a /= 5
        m_r_2 /= 5
        ...
        print(len(topk['4']))
        print(len(topk['5']))
        for j in range(0, len(topk['1'])):
        	if topk['1'][j][0] in topk['1'][j][1]:
        		s1 = s1 +1
        	if topk['2'][j][0] in topk['2'][j][1:3]:
        		s2 = s2 +1
        	if topk['3'][j][0] in topk['3'][j][1:4]:
        		s3 = s3 +1
        	if topk['4'][j][0] in topk['4'][j][1:5]:
        		s4 = s4 +1
        	if topk['5'][j][0] in topk['5'][j][1:6]:
        		s5 = s5 +1
        ...
        print("Turn: ",t)
        print("Matching rate 1: ", m_r)
        print("Matching rate 2: ",m_r_2)
        print("Disease classifier Accuracy: ",acc)
        print("Reward: ",r)
        print("Repeated action: ",r_a)
        print("Loss: ",l)
        print('Top 1 Accuracy : ',s1)
        print('Top 2 Accuracy : ',s2)
        print('Top 3 Accuracy : ',s3)
        print('Top 4 Accuracy : ',s4)
        print('Top 5 Accuracy : ',s5)
